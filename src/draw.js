...
Draw.prototype.initialize = function () {
    ...
    canvas.width  = this.pointDraw.width = this.map.getSize().width
    canvas.height = this.pointDraw.height = this.map.getSize().height
    canvas.style.position = 'absolute'
    canvas.style.top = 0
    canvas.style.left = 0
    this.ctx = canvas.getContext('2d')
    this.pointDraw = new pointDraw(this.map,this.data,this.ctx)
...
Draw.prototype.draw = function ()  {
    ...
    ctx.clearRect(0, 0, this.pointDraw.width, this.pointDraw.height);
